...
def convert_contact_to_grasp_batched(
    ...
    y_axis = normal
    x_axis = torch.cross(gravity, y_axis)
    # Singularity: when surface normal ≈ [0,0,±1] (vertical), cross(gravity, normal) ≈ 0.
    # Detected when |n_z| > 0.98. Fall back to world x-axis then normalize.
    near_vertical = y_axis[:, 2].abs() > 0.98
    if near_vertical.any():
        x_axis[near_vertical, 0] = 1.0
        x_axis[near_vertical, 1] = 0.0
        x_axis[near_vertical, 2] = 0.0
    x_norm = x_axis.norm(dim=-1, keepdim=True).clamp(min=1e-8)
